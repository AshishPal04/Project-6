...
Modified for project 6, including queues and stacks with action functions
*/
...
void Character::strike(Character* target)
{
    ...
    {
        target->armor_ = 0;
        target->vitality_ -= abs(target->armor_ - 2);
    }
}
...
void Character::throwTomato(Character* target)
{
    ...
    {
        target->armor_ = 0;
        target->vitality_ -= abs(target->armor_ - 1);
    }
    int new_vit = getVitality() + 1;
    setVitality(new_vit);
}

/**
@pre: This function is called to  get the action queue of a character
@post: Returns the action queue
*/

queue<int>& Character::getActionQueue()
{
    return action_queue_;
}

/**
@pre: This function is called to get the buff stack of a character
@post: Returns the buff stack
*/

stack<Buff>& Character::getBuffStack()
{
    return buff_stack_;
}